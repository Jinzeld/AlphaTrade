...
async def run_scan_for_ticker(
    ...
    out_channel = bot.get_channel(
        ...
        else config.DISCORD_SWING_CHANNEL
    )
...
async def run_full_scan(trade_type: str, log_channel: discord.TextChannel | None):
...
async def post_settings_panel():
    ...
    await channel.send(
        ...
        "  • 6:30am PST — Trade scans → `#0dte-trades` & `#swing-trades`\n"
        f"  • Every {config.SCAN_INTERVAL_MINUTES}min — Rescan until 1:00pm PST\n"
        "  • 7:00am PST — Long-term scan → `#long-term-picks`\n"
        "  • Every 15min — Breaking news monitor\n"
        "  • 1:00pm PST — Market close\n\n"
        "**📈 Long-Term Scanner**\n"
        "  • Scans Finviz for RSI < 30 stocks daily at 7:00am\n"
        f"  • Market cap filter: `{config.FINVIZ_MIN_MARKET_CAP}`\n"
        "  • Only posts to `#long-term-picks` if STRONG BUY found\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        "`/longterm`               — Run long-term oversold scan now\n\n"
        "**Controls**\n"
        ...
        "`/set-news-interval N`    — Change news check frequency (minutes)\n"
        "`/set-market-cap SIZE`    — Set long-term cap filter (Small/Mid/Large/Mega)\n"
        "`/panel`                  — Show this panel again\n"
        "`/status`                 — Show bot health and config\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )


# ══════════════════════════════════════════════════════════════════════════════
#  Slash commands — watchlist management
# ══════════════════════════════════════════════════════════════════════════════
...
@tree.command(name="scan-0dte", description="Scan one ticker for a 0DTE setup right now")
@app_commands.describe(ticker="Ticker to scan e.g. AAPL")
async def scan_0dte(interaction: discord.Interaction, ticker: str):
    if not is_settings_channel(interaction):
        return await wrong_channel_reply(interaction)
    await interaction.response.send_message(
        f"⚡ Starting 0DTE scan for **{ticker.upper()}**..."
    )
    await run_scan_for_ticker(ticker, "0dte", interaction.channel)


@tree.command(name="scan-swing", description="Scan one ticker for a Swing setup right now")
@app_commands.describe(ticker="Ticker to scan e.g. AAPL")
async def scan_swing(interaction: discord.Interaction, ticker: str):
    if not is_settings_channel(interaction):
        return await wrong_channel_reply(interaction)
    await interaction.response.send_message(
        f"📅 Starting Swing scan for **{ticker.upper()}**..."
    )
    await run_scan_for_ticker(ticker, "swing", interaction.channel)


@tree.command(name="scanall-0dte", description="Scan the full 0DTE watchlist right now")
...
@tree.command(name="scanall-swing", description="Scan the full Swing watchlist right now")
...
@tree.command(name="longterm", description="Run the long-term oversold stock scan right now")
async def longterm_cmd(interaction: discord.Interaction):
    if not is_settings_channel(interaction):
        return await wrong_channel_reply(interaction)
    await interaction.response.send_message(
        f"📊 Running long-term scan — results post in <#{config.DISCORD_LONGTERM_CHANNEL}>..."
    )
    await run_longterm_scan(bot, triggered_by="manual")


# ══════════════════════════════════════════════════════════════════════════════
#  Slash commands — news, reset, settings, status
# ══════════════════════════════════════════════════════════════════════════════
...
@tree.command(name="resume", description="Resume all recurring scans")
...
async def set_news_interval(interaction: discord.Interaction, minutes: int):
    ...
    await interaction.response.send_message(
        f"✅ News interval set to every **{minutes} minutes**.\n"
        f"⚠️ Restart with `docker compose restart` to apply."
    )


@tree.command(name="set-market-cap", description="Set minimum market cap filter for long-term scan")
@app_commands.describe(size="Market cap size: Small, Mid, Large, or Mega")
...
async def reset_bot(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        ...
        "keeps your watchlist exactly as is\n"
...
@tree.command(name="panel", description="Show the settings panel again")
...
@tree.command(name="status", description="Show bot health and current config")
async def status_cmd(interaction: discord.Interaction):
    ...
    longterm_ch = bot.get_channel(config.DISCORD_LONGTERM_CHANNEL)
    ...
    await interaction.response.send_message(
        ...
        f"  📈 Long-term channel: {longterm_ch.mention if longterm_ch else '❌ Not found'}\n\n"
        ...
        f"  ⏰ Scan schedule:     Weekdays 6:30am–1:00pm PST\n"
        f"  📊 Long-term scan:    Weekdays 7:00am PST"
    )
...
async def on_ready():
    await tree.sync()
    logger.info(f"[BOT] Logged in as {bot.user} — slash commands synced")
